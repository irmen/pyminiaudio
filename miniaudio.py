...
class ModplayerError(MiniaudioError):
    """When something goes wrong with the mod players."""
    pass


class ModPlayer:
    """
    Player for Soundtracker/Protracker etc type music tracker mods
    """
    BUFFER_SIZE = 8192

    def __init__(self) -> None:
        self.modctx = ffi.new("jar_mod_context_t *")
        if not lib.jar_mod_init(self.modctx):
            raise ModplayerError("can't init modctx")
        self._state = ffi.new("jar_mod_tracker_buffer_state *")
        self._state.track_state_buf = ffi.new("tracker_state *")
        self._state.nb_max_of_state = 100     # ???
        self._buffer = ffi.new("short[]", ModPlayer.BUFFER_SIZE * 2)
        self.sample_rate = 44100
        self.stereo = True
        self.stereo_separation = True
        self.filter = True
        self.name = ""
        self.instruments = []       # type: List[str]
        if not lib.jar_mod_setcfg(self.modctx, self.sample_rate, 16, self.stereo, self.stereo_separation, self.filter):
            raise ModplayerError("can't set config")

    def set_config(self, sample_rate: int = 44100, stereo: bool = True,
                   stereo_separation: bool = True, filter: bool = True) -> None:
        if not lib.jar_mod_setcfg(self.modctx, sample_rate, 16, int(stereo), int(stereo_separation), int(filter)):
            raise ModplayerError("can't set config")
        self.sample_rate = sample_rate
        self.stereo = stereo
    ...
    def load(self, filename: str) -> int:
        """Loads a mod file"""
        result = lib.jar_mod_load_file(self.modctx, _get_filename_bytes(filename))
        if result <= 0:
            raise ModplayerError("can't load file")
        self.fill_buffer(0, True)  # get initial meta data
        self.name = ffi.string(self._state.name).decode() or os.path.split(filename)[1]
        state = self.current_state()
        self.instruments = [i[1] for i in state.instruments]
        return result

    def close(self) -> None:
        """Unloads mod file and clear player status"""
        lib.jar_mod_unload(self.modctx)

    def seek_start(self) -> None:
        """Reset mod to the beginning?"""
        lib.jar_mod_seek_start(self.modctx)

    def fill_buffer(self, num_stereo_samples: int = BUFFER_SIZE, query_state: bool = False):
        nchannels = 2 if self.stereo else 1
        buff_samples = len(self._buffer) / nchannels
        if num_stereo_samples < 0 or num_stereo_samples > buff_samples:
            raise ModplayerError("invalid number of samples")
        lib.jar_mod_fillbuffer(self.modctx, self._buffer, num_stereo_samples, self._state if query_state else ffi.NULL)

    def get_buffer(self, num_stereo_samples: int = BUFFER_SIZE) -> array.array:
        size = (2 if self.stereo else 1) * num_stereo_samples
        return array.array('h', self._buffer[0:size])

    def current_samples(self) -> int:
        return lib.jar_mod_current_samples(self.modctx)

    def current_state(self) -> "ModPlayer.State":
        state = ModPlayer.State()
        state.current_samples = self.current_samples()
        state.nb_max_of_state = self._state.nb_max_of_state
        state.cur_rd_index = self._state.cur_rd_index
        state.sample_step = self._state.sample_step
        state.load_instruments(self._state.instruments)
        state.load_tracker(self._state.track_state_buf)
        return state
    ...
    class State:
        ...
        sample_step = 0
        current_samples = 0
        instruments = []    # type: List[Tuple[bool, str]]
        tracker = {}        # type: Dict[str, int]
        tracks = []         # type: List[Dict[str, int]]

        def load_instruments(self, instruments: ffi.CData) -> None:
            self.instruments = [(i.active, ffi.string(i.name).decode()) for i in instruments]

        def load_tracker(self, tstate: ffi.CData) -> None:
            self.tracker.clear()
            for d in dir(tstate):
                self.tracker[d] = getattr(tstate, d)
            del self.tracker["tracks"]
            self.tracks = [self._trackstat(t) for t in tstate.tracks]

        def _trackstat(self, track: ffi.CData) -> Dict[str, int]:
            result = {}
            for d in dir(track):
                result[d] = getattr(track, d)
            return result
